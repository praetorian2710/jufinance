...
class Screener:
    # def __init__(self, ticker): The constructor initializes the Screener class with a ticker, which is the stock symbol for the company whose financial data you want to fetch.
    ...
    def stock_information(self):
        ...
        sector_industry = [
            ...
            for term in re.split(
                "\s\s", sector.find_all(class_="sub")[
                    0].text.replace("\n", "").strip()
            )
            if term != ""
        ]
